...
課題指定の更新則に対して, 本実装では1点だけ変更を加えている. **式(5)の更新に勾配ノルムの上限 (クリッピング) を設けた**. 1エピソードから得られる $\boldsymbol{\delta}$ の分布は裾が非常に重く (4.4節), 稀に外れ値が $\boldsymbol{\Delta}$ を支配する. 式(5)は $\boldsymbol{\Delta}$ を正規化せずそのまま $\alpha$ 倍して加算するため, このとき通常の数百倍の更新がそのまま適用されて学習が発散する.

上限を設けずに安定化課題 (4.2節の設定) を学習させ, 方策改善の瞬間の更新量 $\alpha\|\boldsymbol{\Delta}\|$ を記録すると, その中央値は $0.80$ である一方, 99パーセンタイルは $38$, 最大は $282$ に達した. 初期方策のパラメータのノルムが $6$ 程度 ($\theta_i \sim U[-5,5]$ より) であることを考えると, 後者は方策を一撃で壊す大きさである. しかもそのような更新は移動平均に取り込んだ推定値の本数 $n$ が小さい段階に集中しており, $\|\boldsymbol{\Delta}\| > 10$ となった更新の $n$ は中央値 $23$ (全更新の中央値は $183$) であった. $n$ が小さく $\boldsymbol{\Delta}$ が外れ値に支配されている時期に角度基準が偶然満たされることが引き金になっている.

そこで $\|\boldsymbol{\Delta}\|$ の上限を $3$ とし, 1回の更新量を高々 $\alpha \cdot 3 = 1.5$ に制限した. ただしこの上限は稀な外れ値だけを削るものではない. 上限を入れて学習させたときの方策改善時の $\|\boldsymbol{\Delta}\|$ は中央値 $14.8$ であり, 大半の更新で上限が効いている. すなわち実質的には, 勾配の向きのみを使って一定の歩幅で進む正規化つきの勾配上昇に近い更新になっている. これにより安定化課題では発散が完全に消え, swing-up課題でも導入前の16試行中10試行から0〜2試行へ減った (完全には消えない. 3.4節で述べる実行ごとの変動により, 同一設定でも発散する試行数は変わる). 学習係数は $\alpha = 0.5$ とした.
...
方策の対数尤度 (式(2)) の勾配 (score) は `jax.grad` による自動微分でも得られるが, 全レーンをまとめて評価するために解析形を導いて実装している. 実装にあたっては自動微分の値と一致することを確認した.
...
実験の実行と作図を担う. 16個のseedをまとめて学習させる `train_policy_gradient` (方策勾配法) と `train_analytic` (BPTT), 学習後の方策を実環境で評価して維持時間・成功率・割引収益を出力する `report_evaluation`, および各図を描く関数からなる. 実行すると, 報酬設計の比較 (4.1–4.2節), 方策勾配法とBPTTの比較 (4.3節), 勾配推定量の分散の測定 (4.4節), モデル誤差の影響 (4.5節), swing-up課題 (5章) が順に走り, 本レポートの図と主要な数値が再生成される.

### 3.4 並列化と逐次実装との対応

方策勾配法の1ステップあたりの計算はスカラー数個の演算にすぎないため, 逐次に実行すると計算資源をほとんど活用できない. そこで1つの方策パラメータを共有する $N$ 本の環境を `jax.vmap` で同時に進めた. 1イテレーションで $N$ ステップが進むため, 逐次実装に対してイテレーション数が $1/N$ になる.

この並列化により, 逐次実装との差が2点生じる. 第一に, 同一イテレーションで複数のエピソードが同時に終了しうる. 本実装ではそれらの $\boldsymbol{\delta}$ の平均を1件の推定値として式(4)へ取り込んでいる. 後述する通り方策改善のたびに全レーンをリセットするため, 全レーンのエピソードは同期して同時に終了する. したがって**1件の勾配推定値に入るエピソード数は並列環境数に一致する**. すなわち並列環境数 $N$ がそのまま推定値のバッチサイズであり, これを増やしても1ステップあたりの計算は変わらない (レーンはベクトル化されて同時に進むため) 一方, 1回の方策改善に必要な環境ステップ数は比例して増える. 本実験では $N = 4096$ とした. 第二に, より本質的な問題として, **方策を更新した瞬間に走行中のエピソードが古い方策で蓄積した $\boldsymbol{z}$, $\boldsymbol{\delta}$ を持ち越してしまう**. 逐次実装では方策の更新は必ずエピソードの境界で起こるためこの状況は発生しない. $N$ 本の環境を並列に進めていると, 1回の更新のたびに $N-1$ 本のエピソードが古い方策の寄与を含むことになり, 方策勾配法は方策オン型であるからこの汚染は致命的である. 実際, これを放置した実装では学習後の維持時間の中央値が92ステップにとどまり, 学習をいくら長くしても改善しなかった. 方策改善時に走行中のエピソードも破棄するようにしたところ, 同一条件で中央値が488ステップへ改善した.
...
学習に要する時間は, 安定化課題が9.6億ステップで15秒, swing-up課題が80億ステップで8.9分である (いずれも16試行を同時に学習, GPUはRTX 3090). 1イテレーションあたりの処理はスカラー数個の演算であり, 実測スループットは安定化課題で10.1億 env-step/秒に達する. swing-up課題では2.4億 env-step/秒とこれより4倍遅い. これは方策のパラメータが46個あり, 適格度 $\boldsymbol{z}$ と $\boldsymbol{\delta}$ (いずれも レーン数 $\times$ パラメータ数 の配列) の読み書きがGPUのメモリ帯域を律速するためである.

## 4 結果

### 4.1 課題指定の報酬設計の縮退

まず課題に指定された報酬 $r = -s'Qs - aRa$ をそのまま用いて学習した. 結果は16試行のあいだで**二極化する**. 学習後の貪欲方策の維持時間は中央値10.00秒 (評価上限) だが, 四分位範囲は0.67–10.00秒であり, 成功率の四分位範囲にいたっては0.00–1.00である. すなわち一方の端は0.7秒でポールを倒し, 他方の端は10秒間を完全に維持しており, 中間的な挙動はほとんどない.

線形方策 $a = k's$ の空間で割引収益を交差エントロピー法により直接最大化すると, 得られる最適ゲインは $k = (-3.75, -9.57, -1.89, 1.59)$ で, その挙動は0.60秒でポールを倒すというものである. 各制御則について, 学習が最大化している目的関数 (学習時のエピソード上限と同じ200ステップにわたる割引収益) を測ると次のようになる. 維持ステップは1.1節の評価条件 (上限600ステップ) での平均である.

| 方策 | 割引収益 (目的関数) | 平均維持ステップ |
|---|---|---|
| 交差エントロピー法の最適解 (倒れる) | $-2.275$ | 36 |
| 何もしない方策 $a = 0$ (倒れる) | $-2.386$ | 33 |
| LQR | $-3.754$ | 600 (評価上限) |
| 方策勾配法 (16試行平均) | $-3.922$ | 337 |

**倒立を維持するどの制御則よりも, 倒れてしまう方策の方が目的関数の値が高い. 力を一切加えない方策でさえLQRを上回る.** 報酬が常に負である一方, 終端後の収益は0であるから, 早期に終端することが割引収益 $\sum_t \gamma^t r_t$ を増やすためである. 上で見た二極化は, 一方が大域最適である自滅解に到達した試行, 他方が倒立側の局所解に留まった試行であり, 前者の維持時間0.7秒は交差エントロピー法が見つけた最適解 (0.60秒) とほぼ一致する. とくに $\gamma = 0.95$ と $\tau = 1/60$ の組み合わせでは実効的な時間視野が0.33秒しかなく, ポールを維持し続けることの価値がほとんど評価されない. 10秒間の無割引累積報酬で比べると, 倒れる方策の $-6.07$ に対しLQRは $-89.67$ である. 倒立を維持する限り毎ステップ費用が発生し続けるのに対し, 倒れてしまえば費用の累積はそこで止まる.
...
**図1** 課題指定の報酬 (赤) と終端ペナルティを加えた報酬 (青) での学習曲線. 実線は16試行の中央値, 帯は四分位範囲. 横軸はエピソード数. (a) 1エピソードあたりの累積報酬 (比較のため終端ペナルティを除いて表示), (b) 1エピソードの長さ. 学習中は行動雑音 $\sigma$ を乗せた確率的方策で走らせているため, 決定論的方策の維持時間 (評価上限の600ステップ) よりエピソードは短くなる.

### 4.2 終端ペナルティの導入と学習曲線

そこで, 終端した時点で報酬 $-20$ を追加で与える設計を導入した. これは失敗に明示的なコストを与える標準的な手法であり, 上述の縮退を除去する. 実際, 同じ交差エントロピー法による直接探索の最適ゲインは $k = (-0.61, 4.27, 48.95, 15.73)$ となり, 式(8)のLQRゲインとよく一致する. すなわちこの報酬設計のもとでは, 本来意図された安定化制御が大域最適解になっている.

図1の青線がこの設定での学習曲線である. 学習後の貪欲方策の性能は, 維持時間が10.00秒 (評価上限, 四分位範囲 9.98–10.00秒), 成功率が1.00 (四分位範囲 0.99–1.00) であり, 4.1節で見た二極化は消えて全16試行が安定化制御を獲得した. 消費した実環境のエピソード数は4,876,363, 方策改善の回数は272回, 学習に要した時間は15秒である.

課題に「10万エピソード以上を要する」とある通り, 100万エピソード規模の経験を要している. なお本実装では並列環境数がそのまま勾配推定値のバッチサイズになるため (3.4節), 1回の方策改善に多数のエピソードを消費する. 消費エピソード数が課題の想定より多いのはこのためであり, 実時間では15秒で完了する.
...
最終的な制御性能はいずれも同等で, 維持時間はBPTTが10.00秒 (成功率 1.00, 四分位範囲 0.99–1.00), 方策勾配法も10.00秒 (成功率 1.00, 同 0.99–1.00) である. 差はサンプル効率に現れる. BPTTは約 $10^4$ 本の模擬エピソードでLQRと同水準の割引収益 $-4.573$ に到達するのに対し, 方策勾配法は $4.9 \times 10^6$ 本の実環境エピソードを要する. およそ2桁以上の差がある.
...
学習された制御則をゲインの水準で比較すると, 方策勾配法が $k = (1.921, 0.753, 81.871, 13.340)$, BPTTが $k = (0.469, -0.026, 35.710, 5.618)$, LQRが $k = (0.385, 1.833, 46.081, 11.203)$ である. いずれも角度に対するゲイン $k_3$ が支配的である点は共通するが, その値は $35.7$–$81.9$ と手法によって2倍以上の開きがあり, 角速度ゲイン $k_4$ もBPTTはLQRの半分, 方策勾配法はLQRより大きい. それにもかかわらず10秒間の維持性能はいずれも同等であった. すなわち本課題では最適解の近傍で目的関数が平坦であり, ゲインの水準では相当に異なる解が同等の性能を与える. LQRから離れる要因としては, (i)課題の目的関数が有限ホライズンかつ終端をもつためLQRが厳密な最適解ではないこと, (ii)方策が確率的であり $\sigma \ge 0.1$ の行動雑音が常に乗るため, 大きなゲインは雑音を増幅して費用を増やすことの2点が考えられる.
...
3手法とも初期角度を約1秒で $0$ 付近へ戻し, 以後は角度をほぼ $0$ に保つ. 差はカートの挙動に現れる. LQRはカートを $0.6$ m まで動かしたのち原点へ戻していくのに対し, BPTTはカート速度に対するゲイン $k_2 \approx 0$ のため位置を戻す作用が弱く, 10秒間で $-0.95$ m まで漂流し続ける. 方策勾配法は $k_3$ が大きいぶん初期の入力が最大 $11$ N とLQR ($6.5$ N) より大きく, 角度をより速く戻す. いずれの軌道も終端条件には触れないため, この違いは評価上の性能差にはならない. 角度に関する費用 $Q_{33} = 12$ が支配的な報酬のもとでは, カート位置の緩やかな漂流はほとんど罰せられないためである.
...
- **方策勾配法** はモデルを一切用いず, 実環境との相互作用のみから収益を最大化する. 本課題では約488万エピソードを要するが, 得られた制御則は他の2手法と同等である. モデル誤差の影響を原理的に受けない.
...
| 方策勾配法 | $176.4 \pm 83.0$ | 0.06 | **11.68 周** | 4493万 (実環境) |
| BPTT ($\phi = C\boldsymbol{s}$, 課題指定の特徴) | $120.4 \pm 92.9$ | 0.05 | 12.47 周 | 76.8万 (模擬) |
| LQR | 38.0 | 0.03 | 0.65 周 | 0 |

**表2** swing-up課題の結果 (方策勾配法とBPTTは各16試行, LQRは決定論的なので1つ. 倒立滞在率は最終100ステップで $|y| < 12°$ にある割合. 回転数は角度を折り返さずに数えた周回数で, 静止なら0, 大振幅の往復なら1前後, 回転していれば数周以上になる. 本実行では発散した試行はなかったが, 方策勾配法は試行ごとのばらつきが大きく標準偏差が83に達する. なお図5(a)の縦軸は学習中の確率的方策の収益を試行の中央値でまとめたものであり, 学習後の決定論的方策を評価した表の値とは一致しない).

**BPTT** は振り上げに成功し, 倒立近傍に長く滞在する. 最終100ステップのうち $|y| < 12°$ にある割合は0.33であり, 4手法のなかで唯一, 倒立点に有意な時間とどまる. しかし頂点で静定できず, 後半350ステップ (5.8秒) で約0.8周ぶんの往復を続ける. 回転数が1周に満たないことは「回転している」のではなく「頂点を越えてはゆっくり戻ってくる」状態であることを意味する. 図5(b)の緑線でも, 5.5秒付近で角度が $0°$ に達したのち, 静止せずにふたたび下方へ落ちていく様子が見える. 16試行の標準偏差が5.5と小さく, 全試行が同じ挙動に収束する.

**方策勾配法** は質的に異なる失敗をする. 後半350ステップで11.7周しており, 平均角速度にすると約12.6 rad/s である. 倒立滞在率は0.06しかなく, 一様回転そのものである. すなわちポールを回し続ける局所解に収束しており, 振り上げ動作自体を獲得できていない. 図5(b)の青線が一定の傾きで折り返しを繰り返す鋸歯状の波形になっているのがこの挙動である.
...
学習量を増やしても解決しない. 8億ステップ (方策改善57回) で収益125, 32億ステップ (341回) で166, 80億ステップ (792回) で176と, 学習量を10倍にしても1.4倍にしかならず飽和している. 更新回数が不足しているのではなく, 局所解に到達して止まっている.
